(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),t.credentials=e.crossOrigin===`use-credentials`?`include`:e.crossOrigin===`anonymous`?`omit`:`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})(),((e,t)=>()=>(t||(e((t={exports:{}}).exports,t),e=null),t.exports))((()=>{var e={token:localStorage.getItem(`erp_token`)||null,user:JSON.parse(localStorage.getItem(`erp_user`)||`null`),currentTab:`dashboard`,activeClientSubTab:`active`,dashboard:null,clients:[],activeClientsCount:0,inactiveClientsCount:0,clientSearchQuery:``,employees:[],salaries:[],salaryMonth:new Date().getMonth()+1,salaryYear:new Date().getFullYear(),expenses:[],expenseMonth:new Date().getMonth()+1,expenseYear:new Date().getFullYear(),expenseCategories:[],expenseTotals:{totalGeneral:0,totalSalaryPaid:0,totalDeductible:0},workTracker:[],activeModal:null,modalPayload:{},toast:null,loading:!1};async function t(t,r={}){let a={"Content-Type":`application/json`,...r.headers||{}};e.token&&(a.Authorization=`Bearer ${e.token}`);try{let e=await fetch(t,{...r,headers:a}),n=await e.json();if(!e.ok)throw e.status===401&&i(),Error(n.error||`Request failed`);return n}catch(e){throw n(e.message,`error`),e}}function n(t,n=`success`){e.toast={message:t,type:n},l(),setTimeout(()=>{e.toast=null,l()},4e3)}async function r(r,i){e.loading=!0,l();try{let o=await t(`/api/auth/login`,{method:`POST`,body:JSON.stringify({login:r,password:i})});e.token=o.token,e.user=o.user,localStorage.setItem(`erp_token`,o.token),localStorage.setItem(`erp_user`,JSON.stringify(o.user)),e.currentTab=`dashboard`,n(`Welcome back, ${o.user.name}!`),await a()}catch{}finally{e.loading=!1,l()}}function i(){e.token=null,e.user=null,localStorage.removeItem(`erp_token`),localStorage.removeItem(`erp_user`),l()}async function a(){if(e.token){e.loading=!0,l();try{if(e.currentTab===`dashboard`)e.dashboard=await t(`/api/dashboard`);else if(e.currentTab===`clients`){let n=await t(`/api/clients`);e.clients=n.clients,e.activeClientsCount=n.activeCount,e.inactiveClientsCount=n.inactiveCount}else if(e.currentTab===`employees`)e.employees=(await t(`/api/employees`)).employees;else if(e.currentTab===`salary`)e.salaries=(await t(`/api/salary?month=${e.salaryMonth}&year=${e.salaryYear}`)).salaries;else if(e.currentTab===`expenses`){let n=await t(`/api/expenses?month=${e.expenseMonth}&year=${e.expenseYear}`);e.expenses=n.expenses,e.expenseCategories=n.categories,e.expenseTotals={totalGeneral:n.totalGeneral,totalSalaryPaid:n.totalSalaryPaid,totalDeductible:n.totalDeductible}}else if(e.currentTab===`work-tracker`){let n=await t(`/api/work-tracker`);e.workTracker=n.assignments,e.employees=n.employees}}catch(e){console.error(e)}finally{e.loading=!1,l()}}}function o(t){e.currentTab=t,e.activeModal=null,a()}function s(e){return`₹`+parseFloat(e||0).toLocaleString(`en-IN`,{minimumFractionDigits:2,maximumFractionDigits:2})}function c(e){if(!e)return`—`;try{return new Date(e).toLocaleDateString(`en-IN`,{day:`2-digit`,month:`2-digit`,year:`numeric`})}catch{return e}}function l(){let t=document.getElementById(`app`);if(!e.token){t.innerHTML=u(),lucide.createIcons(),C();return}t.innerHTML=`
  ...
  `,lucide.createIcons(),w()}function u(){return`
  ...
  `}function d(){return`
    <aside class="w-full md:w-64 lg:w-72 bg-slate-950 border-r border-slate-800/80 flex flex-col flex-shrink-0">
      ...
      <nav class="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
        ${[{id:`dashboard`,label:`Executive Dashboard`,icon:`layout-dashboard`},{id:`clients`,label:`Clients Hub`,icon:`users`,badge:e.activeClientsCount||null},{id:`employees`,label:`Employees`,icon:`user-check`},{id:`salary`,label:`Salaries & Payouts`,icon:`wallet`},{id:`expenses`,label:`Unified Expenses`,icon:`receipt`},{id:`work-tracker`,label:`Work Tracker`,icon:`calendar-check`}].map(t=>`
          <button onclick="window.switchTab('${t.id}')" 
                  class="w-full flex items-center justify-between px-4 py-3 rounded-xl text-sm font-semibold transition-all ${e.currentTab===t.id?`bg-indigo-600 text-white shadow-lg shadow-indigo-600/25`:`text-slate-400 hover:text-slate-100 hover:bg-slate-900`}">
            <div class="flex items-center gap-3">
              <i data-lucide="${t.icon}" class="w-4 h-4"></i>
              <span>${t.label}</span>
            </div>
            ${t.badge?`<span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-indigo-950 text-indigo-300 border border-indigo-700/50">${t.badge}</span>`:``}
          </button>
        `).join(``)}
      </nav>
  ...
  `}function p(){return e.loading&&!e.dashboard&&!e.clients.length?`
    ...
    `:e.currentTab===`dashboard`?m():e.currentTab===`clients`?h():e.currentTab===`employees`?g():e.currentTab===`salary`?_():e.currentTab===`expenses`?v():e.currentTab===`work-tracker`?y():`<div>Tab not found</div>`}function m(){let t=e.dashboard||{};return`
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        ...
        <div class="glass-card rounded-2xl p-5 border border-rose-500/20 bg-gradient-to-br from-rose-950/20 to-slate-900">
          ...
          <p class="text-[11px] text-slate-400 mt-1">${t.activeDueClientsCount||0} active clients pending</p>
        </div>
        ...
        <div class="glass-card rounded-2xl p-5 border border-emerald-500/20 bg-gradient-to-br from-emerald-950/20 to-slate-900">
          ...
          <p class="text-[11px] text-slate-400 mt-1">Today: <span class="font-bold text-white">${s(t.todayCollection)}</span></p>
        </div>
        ...
        <div class="glass-card rounded-2xl p-5 border border-amber-500/20 bg-gradient-to-br from-amber-950/20 to-slate-900">
          ...
          <p class="text-[11px] text-slate-400 mt-1">Commission: <span class="font-bold text-white">${s(t.totalCommissionThisMonth)}</span></p>
        </div>
        ...
        <div class="glass-card rounded-2xl p-5 border border-indigo-500/20 bg-gradient-to-br from-indigo-950/20 to-slate-900">
          ...
          <p class="text-2xl font-black text-indigo-400">${s(t.availableFund)}</p>
          ...
          <div class="space-y-3">
            ...
            ${(t.upcomingRenewals||[]).map(e=>`
              <div class="flex items-center justify-between p-3.5 rounded-2xl bg-slate-900/80 border border-slate-800">
                <div>
                  <h4 class="text-sm font-bold text-white">${e.client_name}</h4>
                  <p class="text-xs text-slate-400">Expires: <span class="text-amber-400 font-semibold">${c(e.end_date)}</span> • ${e.client_mobile}</p>
                </div>
                <button onclick="window.openRenewModal(${e.client_id}, '${e.client_name}', ${e.package_amount})" class="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold">
                  🔄 Renew
                </button>
              </div>
            `).join(``)}
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2 p-1 rounded-2xl bg-slate-900 border border-slate-800 w-fit">
          <button onclick="window.setClientSubTab('active')" class="px-4 py-2 rounded-xl text-xs font-bold transition-all ${e.activeClientSubTab===`active`?`bg-indigo-600 text-white shadow-md`:`text-slate-400 hover:text-white`}">
        ...
        <div class="flex items-center gap-3">
          <div class="relative">
            ...
            <input type="text" oninput="window.setClientSearch(this.value)" value="${e.clientSearchQuery}" placeholder="Search client..." class="pl-10 pr-4 py-2 rounded-xl bg-slate-900 border border-slate-800 text-xs text-white focus:outline-none focus:border-indigo-500 w-48 sm:w-64">
          </div>
          ...
          <div class="glass-panel rounded-3xl p-5 border border-slate-800 hover:border-indigo-500/40 transition-all flex flex-col justify-between">
            <div>
              <div class="flex items-start justify-between gap-2 mb-3">
                ...
                <span class="px-2.5 py-1 rounded-full text-[10px] font-bold ${e.status===`active`?`bg-emerald-500/10 text-emerald-400 border border-emerald-500/20`:`bg-amber-500/10 text-amber-400 border border-amber-500/20`}">
            ...
            <div class="flex items-center gap-2 pt-2 border-t border-slate-800/60">
              <button onclick="window.openRenewModal(${e.id}, '${e.name}', ${e.current_package})" class="flex-1 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1">
                🔄 Renew
              </button>
  ...
  `}function g(){return`
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <h2 class="text-base font-bold text-white">Active Employees (${e.employees.length})</h2>
              ...
              ${e.salaries.map(e=>`
                <tr class="hover:bg-slate-900/40 transition-all">
                  <td class="p-4">
                    ...
                    <p class="text-[11px] text-slate-500">${e.employee?.role_title||``}</p>
                  </td>
                  <td class="p-4 font-semibold text-slate-300">${s(e.base_salary)}</td>
                  <td class="p-4 font-semibold text-indigo-400">${s(e.total_commission)}</td>
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="px-3.5 py-2 rounded-2xl bg-slate-900 border border-slate-800 text-xs">
            ...
            <span class="font-bold text-indigo-400 ml-1.5">${s(e.expenseTotals.totalSalaryPaid)}</span>
          </div>
          <div class="px-3.5 py-2 rounded-2xl bg-slate-900 border border-slate-800 text-xs">
              ...
              ${e.workTracker.map(e=>`
                <tr class="hover:bg-slate-900/40 transition-all">
                  <td class="p-4">
                    ...
                    <p class="text-[11px] text-slate-500">${e.client_mobile}</p>
                  </td>
                  <td class="p-4 font-semibold text-indigo-300">${e.employee_name}</td>
                  <td class="p-4 text-amber-400 font-semibold">${c(e.assigned_date)}</td>
                  <td class="p-4 text-slate-400">${e.work_location||`—`}</td>
                  <td class="p-4 text-right">
                    <button onclick="window.openWorkHistoryModal(${e.client_id}, '${e.client_name}')" class="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold">
                      📅 Month History
                    </button>
                  </td>
                </tr>
  ...
  `}function b(){return e.activeModal?`
  ...
  `:``}function x(){let t=e.activeModal,n=e.modalPayload||{};if(t===`addClient`)return`
    ...
    `;if(t===`renewClient`){let e=new Date().toISOString().split(`T`)[0],t=new Date(new Date().setMonth(new Date().getMonth()+1)).toISOString().split(`T`)[0];return`
      <h3 class="text-lg font-bold text-white mb-2">🔄 Renew Client — ${n.clientName}</h3>
    ...
    `}return t===`receivePayment`?`
      <h3 class="text-lg font-bold text-white mb-2">💰 Receive Payment — ${n.clientName}</h3>
    ...
    `:``}function S(){if(!e.toast)return``;let t=e.toast.type===`error`;return`
