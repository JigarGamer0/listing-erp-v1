...
<head>
    ...
    <title>@yield('title', 'Dashboard') — {{ \App\Models\Setting::get('company_name', 'Listing ERP') }}</title>
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="sidebar-transition fixed inset-y-0 left-0 z-50 flex flex-col bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 text-gray-800 dark:text-gray-200"
            ...
            <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-1.5" x-data="{
                pinnedCategories: {
                    'menu-clients': {{ request()->routeIs('clients.*') || request()->routeIs('payments.*') ? 'true' : 'false' }},
                    'menu-employees': {{ request()->routeIs('employees.*') || request()->routeIs('salary.*') || request()->routeIs('admin.advances*') || request()->routeIs('admin.holidays*') ? 'true' : 'false' }},
                    'menu-investors': {{ request()->routeIs('investors.*') || request()->routeIs('investments.*') ? 'true' : 'false' }}
                },
                ...
                @foreach($sidebarMenu as $item)
                    ...
                    @if($item['type'] === 'category')
                        <div class="space-y-1"
                            ...
                            <button @click="pinnedCategories['{{ $item['id'] }}'] = !pinnedCategories['{{ $item['id'] }}']"
                                ...
                                <div class="flex items-center">
                                    ...
                                    <span class="ml-3 text-sm font-medium" x-show="sidebarOpen" x-transition>{{ $item['label'] }}</span>
                                </div>
                                <svg class="w-4 h-4 transition-transform duration-200" :class="pinnedCategories['{{ $item['id'] }}'] || hoveredCategory === '{{ $item['id'] }}' ? 'transform rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24" x-show="sidebarOpen" x-transition>
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                                </svg>
                            </button>
                            
                            <div x-show="pinnedCategories['{{ $item['id'] }}'] || hoveredCategory === '{{ $item['id'] }}'" x-transition class="pl-9 pr-2 py-1 space-y-1">
                                @foreach($item['children'] as $child)
                                    ...
                                    @endphp
                                    <a href="{{ route($child['route']) }}{{ isset($child['anchor']) ? $child['anchor'] : '' }}"
                                       class="block px-3 py-2 text-xs rounded-lg transition-all duration-150 
                                              {{ $isActive ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-300 font-semibold' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white' }}">
                                        {{ $child['label'] }}
                                    </a>
                                @endforeach
                            </div>
                    ...
                    @else
                        <a href="{{ route($item['route']) }}"
                           class="flex items-center px-4 py-3 rounded-xl transition-all duration-200 group
                                  {{ request()->routeIs($item['active']) ? 'bg-blue-600 text-white shadow-md shadow-blue-500/20 font-medium' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white' }}">
                            <svg class="w-5 h-5 flex-shrink-0 transition-colors {{ request()->routeIs($item['active']) ? 'text-white' : 'text-gray-400 group-hover:text-gray-700 dark:group-hover:text-gray-200' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $item['icon'] }}"/>
                            </svg>
                            <span class="ml-3 text-sm" x-show="sidebarOpen" x-transition>{{ $item['label'] }}</span>
                        </a>
                    @endif
                @endforeach
            </nav>
            ...
            {{-- Quick Stats Widget --}}
            @if(request()->routeIs('clients.*') || request()->routeIs('dashboard'))
            <div class="px-5 py-4 mx-4 mb-4 bg-gray-50/50 dark:bg-gray-850 rounded-2xl border border-gray-100 dark:border-gray-800" x-show="sidebarOpen" x-transition>
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-xs font-bold text-gray-800 dark:text-white">{{ \App\Models\Client::count() }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-2 text-xs text-gray-500 dark:text-gray-400">
                        ...
                        <span class="text-xs font-bold text-gray-800 dark:text-white">{{ \App\Models\Client::where('status', 'active')->count() }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-2 text-xs text-gray-500 dark:text-gray-400">
                        ...
                        <span class="text-xs font-bold text-gray-800 dark:text-white">{{ \App\Models\Client::where('status', 'inactive')->count() }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-2 text-xs text-gray-500 dark:text-gray-400">
                        ...
                        <span class="text-xs font-bold text-red-600">₹{{ number_format(\App\Models\ClientBillingCycle::whereIn('status', ['pending', 'partial', 'overdue'])->sum('balance'), 0) }}</span>
                    </div>
                </div>
            </div>
            @endif
                    ...
                    <div x-data="{ notifOpen: false }" class="relative">
                        ...
                        <div x-show="notifOpen" @click.outside="notifOpen = false" x-transition
                            ...
                            <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                                ...
                                <a href="{{ route('notifications.index') }}" class="text-blue-600 text-sm hover:underline">View All</a>
                            </div>
