...
<div class="fade-in" x-data="{ showDeleteModal: false, deleteUrl: '', deleteName: '', showAddEmployeeModal: false, createLogin: false }">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden shadow-sm">
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    @forelse($employees as $emp)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 cursor-pointer transition-all" onclick="if(!event.target.closest('a') && !event.target.closest('button')) window.location='{{ route('employees.show', $emp) }}'">
                            <td class="px-5 py-3"><a href="{{ route('employees.show', $emp) }}" class="text-sm font-medium text-blue-600 hover:underline">{{ $emp->name }}</a></td>
                            <td class="px-5 py-3 text-sm text-gray-600 dark:text-gray-400">{{ $emp->phone ?? '—' }}</td>
                            <td class="px-5 py-3 text-sm text-gray-600 dark:text-gray-400">{{ ucfirst(str_replace('_', ' ', $emp->salary_type)) }}</td>
                            <td class="px-5 py-3 text-sm text-right text-gray-800 dark:text-white">₹{{ number_format($emp->total_salary_estimate, 0) }}</td>
                            <td class="px-5 py-3 text-sm text-right text-gray-800 dark:text-white">{{ $emp->total_pending_advance > 0 ? '₹' . number_format($emp->total_pending_advance, 0) : '-' }}</td>
                            <td class="px-5 py-3 text-sm text-right font-semibold text-red-600">₹{{ number_format($emp->pending_salary, 0) }}</td>
                            <td class="px-5 py-3 text-sm text-gray-600 dark:text-gray-400">{{ $emp->commission_type === 'percentage' ? $emp->commission_value . '%' : '₹' . number_format($emp->commission_value, 0) }}</td>
                            <td class="px-5 py-3 text-sm text-center text-gray-800 dark:text-white">{{ $emp->activeAssignments->count() }}</td>
                            <td class="px-5 py-3 text-center"><span class="px-2 py-0.5 rounded-full text-xs font-medium {{ $emp->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">{{ ucfirst($emp->status) }}</span></td>
                            <td class="px-5 py-3 text-center whitespace-nowrap">
                                <a href="{{ route('employees.show', $emp) }}" title="View" class="p-1.5 hover:bg-blue-50 dark:hover:bg-blue-900/20 text-blue-600 rounded-lg inline-block transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </a>
                                <a href="{{ route('employees.edit', $emp) }}" title="Edit" class="p-1.5 hover:bg-amber-50 dark:hover:bg-amber-900/20 text-amber-600 rounded-lg inline-block transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </a>
                                <button type="button" 
                                        @click="deleteUrl = '{{ route('employees.destroy', $emp) }}'; deleteName = '{{ addslashes($emp->name) }}'; showDeleteModal = true;" 
        ...
        <div class="px-5 py-4 border-t border-gray-100 dark:border-gray-700">{{ $employees->links() }}</div>
    </div>
