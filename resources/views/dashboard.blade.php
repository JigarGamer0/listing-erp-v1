...
<div class="fade-in" x-data="{
    ...
    payrollPayable: {{ (float)$totalSalaryPayableThisMonth }},
    totalReceivables: {{ (float)$paymentDue }},
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5 mb-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700 shadow-sm relative overflow-hidden">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-extrabold text-gray-900 dark:text-white mt-1">₹{{ number_format($paymentDue, 0) }}</p>
                </div>
            ...
            <div class="mt-4 flex items-center text-xs text-gray-500">
                <span class="px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-950/30 text-amber-600 font-semibold">{{ $activeDueClientsCount }} Active Clients</span>
                <span class="ml-1.5 font-medium">pending due</span>
            </div>
        </div>

        {{-- Card 2: Monthly Payroll Payable --}}
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700 shadow-sm relative overflow-hidden">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs font-bold text-indigo-500 uppercase tracking-wider">Monthly Payroll Payable</p>
                    <p class="text-2xl font-extrabold text-indigo-600 dark:text-indigo-400 mt-1">₹{{ number_format($totalSalaryPayableThisMonth, 0) }}</p>
                </div>
            ...
            <div class="mt-4 flex items-center text-xs text-gray-500 font-medium">
                <span>Fixed + Commission: <strong class="text-gray-700 dark:text-gray-300">₹{{ number_format($totalCommissionThisMonth, 0) }}</strong></span>
            </div>
        </div>

        {{-- Card 3: Monthly Expenses (Clickable to Toggle/Manage) --}}
        ...
        {{-- Card 4: Net Projected Savings (Reactive) --}}
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700 shadow-sm relative overflow-hidden">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-extrabold mt-1" 
                       ...
                       x-text="'₹' + Number(netProjectedSavings).toLocaleString('en-IN')">
                        ₹{{ number_format($netProjectedBachat, 0) }}
                    </p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700 shadow-sm relative overflow-hidden">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-extrabold text-gray-900 dark:text-white mt-1">₹{{ number_format($availableFund, 0) }}</p>
            ...
            <div class="mt-4 flex items-center text-xs text-gray-500 font-medium">
                <span>Total Collection: <strong class="text-gray-700 dark:text-gray-300">₹{{ number_format($monthlyCollection, 0) }}</strong></span>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col justify-between">
            ...
            <div class="grid grid-cols-2 gap-3 flex-1">
                <a href="{{ route('clients.create') }}" class="flex flex-col items-center justify-center p-4 bg-blue-50/50 hover:bg-blue-50 dark:bg-blue-900/10 dark:hover:bg-blue-900/20 rounded-xl transition-all border border-blue-100/50 dark:border-blue-900/20 group">
                ...
                <a href="{{ route('employees.create') }}" class="flex flex-col items-center justify-center p-4 bg-indigo-50/50 hover:bg-indigo-50 dark:bg-indigo-900/10 dark:hover:bg-indigo-900/20 rounded-xl transition-all border border-indigo-100/50 dark:border-indigo-900/20 group">
                ...
                <a href="{{ route('expenses.create') }}" class="flex flex-col items-center justify-center p-4 bg-amber-50/50 hover:bg-amber-50 dark:bg-amber-900/10 dark:hover:bg-amber-900/20 rounded-xl transition-all border border-amber-100/50 dark:border-amber-900/20 group">
                ...
                <a href="{{ route('salary.advance.form') }}" class="flex flex-col items-center justify-center p-4 bg-purple-50/50 hover:bg-purple-50 dark:bg-purple-900/10 dark:hover:bg-purple-900/20 rounded-xl transition-all border border-purple-100/50 dark:border-purple-900/20 group">
                ...
                <a href="{{ route('salary.index') }}" class="flex flex-col items-center justify-center p-4 bg-rose-50/50 hover:bg-rose-50 dark:bg-rose-900/10 dark:hover:bg-rose-900/20 rounded-xl transition-all border border-rose-100/50 dark:border-rose-900/20 group">
        ...
        {{-- Upcoming Renewals / Payment Due list --}}
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col justify-between">
            ...
            <div class="space-y-3 flex-1">
                @forelse($paymentDueClients->take(3) as $client)
                    <div class="flex items-center justify-between py-2 border-b border-gray-50 dark:border-gray-700/50">
                        <div>
                            <p class="text-xs font-semibold text-gray-800 dark:text-gray-200">{{ $client->name }}</p>
                            <p class="text-[10px] text-gray-400">Due Date: {{ now()->addDays(5)->format('d M Y') }}</p>
                        </div>
                        <span class="text-xs font-bold text-amber-600">₹{{ number_format($client->total_due, 0) }}</span>
                    </div>
                @empty
    ...
    {{-- ─── Charts & Activity Row (3 Columns) ─────────────────── --}}
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {{-- Column 1: Revenue vs Expense --}}
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col justify-between">
                ...
                <div class="space-y-4 max-h-48 overflow-y-auto pr-1">
                    @forelse($recentActivities->take(4) as $activity)
                        <div class="flex items-start space-x-3 text-xs">
                            ...
                            <div>
                                <p class="text-gray-700 dark:text-gray-300 font-medium">{{ $activity->description }}</p>
            ...
            <div class="pt-3 border-t border-gray-50 dark:border-gray-700/50 mt-3">
                <a href="{{ route('activity-logs.index') }}" class="w-full inline-flex justify-center items-center py-2 bg-gray-50 dark:bg-gray-700 text-xs font-semibold text-gray-600 dark:text-gray-300 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
                    View All Activities
                </a>
            </div>
        </div>
    ...
    {{-- Monthly Expenses Deduction Manager Modal (Tick/Untick Deductions) --}}
    <div x-show="showExpenseDeductionModal" 
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl max-w-2xl w-full border border-gray-100 dark:border-gray-700 shadow-2xl p-6 relative overflow-hidden" 
            ...
            {{-- Real-time KPI Bar inside modal --}}
            ...
            {{-- List of expenses with interactive checkboxes --}}
...
document.addEventListener('DOMContentLoaded', function() {
    const collectionData = @json($monthlyCollectionData);
    const expenseData = @json($monthlyExpenseData);
    const profitData = @json($monthlyProfitData);
...
<div id="clear-fund-modal" class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-50 hidden">
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-6 max-w-md w-full mx-4 shadow-xl relative">
        ...
        <form method="POST" action="{{ route('fund.clear') }}">
            @csrf
            <div class="space-y-4">
                <div class="p-4 bg-gray-50 dark:bg-gray-700/30 rounded-xl">
                    <p class="text-xs text-gray-500">Current Available Fund</p>
                    <p class="text-xl font-bold text-gray-900 dark:text-white">₹{{ number_format($availableFund, 2) }}</p>
                </div>
                    ...
                    <label class="flex items-center space-x-3 p-3 border border-gray-200 dark:border-gray-700 rounded-xl cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-750/20">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="mt-2 hidden" id="clear_custom_amount_container">
                                <input type="number" step="0.01" min="0.01" max="{{ $availableFund }}" name="amount" id="clear_custom_amount" placeholder="Enter amount (e.g. 5000)" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-650 rounded-xl text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white outline-none">
                            </div>
                        </div>
                    </label>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="date" value="{{ date('Y-m-d') }}" required class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-700 text-gray-800 dark:text-white outline-none text-sm">
                </div>
